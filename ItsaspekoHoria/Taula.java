...
public class Taula {
	...
	public boolean lekuaDago(int luzera, int x, int y, int norabidea) {// 0(gora)
																		// 1(behera)
																		// 2(eskuma)
																		// 3(ezkerra)
		boolean libre = true;
	...
	public void radarKontsultatu(int x, int y) {
		...
		if (x > 0 && taulaMatrizea[x - 1][y].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x - 1][y].koloreAldatu();
		if (x > 0 && y < 9 && taulaMatrizea[x - 1][y + 1].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x - 1][y + 1].koloreAldatu();
		if (y > 0 && taulaMatrizea[x][y - 1].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x][y - 1].koloreAldatu();
		if (y < 9 && taulaMatrizea[x][y + 1].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x][y + 1].koloreAldatu();
		if (x < 9 && y > 0 && taulaMatrizea[x + 1][y - 1].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x + 1][y - 1].koloreAldatu();
		if (x < 9 && taulaMatrizea[x + 1][y].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x + 1][y].koloreAldatu();
		if (x < 9 && y < 9 && taulaMatrizea[x + 1][y + 1].getEgoera() instanceof OntziaDago)
			taulaMatrizea[x + 1][y + 1].koloreAldatu();
